...
install_tools() {
  ...
  sudo apt-get install -y -qq slowhttptest 2>/dev/null || \
    warn "slowhttptest not available — using Python slowloris fallback"
...
trap 'echo -e "\n${RED}Stopping...${RESET}"; kill "${PIDS[@]}" 2>/dev/null; exit 0' INT TERM

echo -e "\n${BOLD}${RED}>> MULTISTRESS v2${RESET} — Linux — no proxy\n"
install_tools
run_all
